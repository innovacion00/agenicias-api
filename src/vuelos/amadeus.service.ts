...
export class AmadeusService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!response.data.access_token || !response.data.expires_in) {
        throw new Error(
          'Respuesta inválida del servidor de autenticación de Amadeus',
        );
      }
      ...
      this.logger.log(
        `Token de Amadeus obtenido exitosamente. Expira en ${response.data.expires_in} segundos`,
      );
      this.logger.debug(
        `Tipo de token: ${response.data.token_type}, Estado: ${response.data.state}`,
      );
      ...
      throw new HttpException(
        'Error al autenticarse con Amadeus: ' +
  ...
  private async makeAuthenticatedRequest<T>(
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `[AMADEUS_RETRY] Token expirado, renovando y reintentando... (intento ${retryCount + 1})`,
          {
            requestId: logContext.requestId,
            endpoint,
            method,
  ...
  async searchLocations(
    params: AmadeusLocationQueryParams,
  ...
  async searchCitiesByName(
    ...
    this.logger.log(
      `Buscando ciudad: ${cityName}${countryCode ? ` en ${countryCode}` : ''}`,
    );
  ...
  async searchCities(params: {
    ...
    this.logger.log(
      `Buscando ciudades con keyword: ${params.keyword}${params.countryCode ? ` en ${params.countryCode}` : ''}`,
    );
  ...
  async searchFlightOffers(
    searchRequest: AmadeusFlightOffersRequest,
    ...
    try {
      this.logger.log(
        `[FLIGHT_SEARCH] Iniciando búsqueda de ofertas de vuelos`,
        {
          requestId: logContext.requestId,
      ...
      const response =
        await this.makeAuthenticatedRequest<AmadeusFlightOffersResponse>(
          'POST',
      ...
      this.logger.log(
        `[FLIGHT_SEARCH_SUCCESS] Búsqueda completada exitosamente`,
        {
          requestId: logContext.requestId,
      ...
      this.logger.error(
        `[FLIGHT_SEARCH_ERROR] Error en búsqueda de ofertas de vuelos`,
        {
          requestId: logContext.requestId,
    ...
    const amadeusRequest: any = {
      ...
      originDestinations: searchRequest.originDestinations.map((od: any) => ({
        ...
        departureDateTimeRange: {
          ...
          time:
            od.departureDateTimeRange?.time || od.departureTime || '00:00:00',
        },
      })),
    ...
    if (searchRequest.searchCriteria) {
      ...
      if (searchRequest.searchCriteria.flightFilters) {
        ...
        if (filters.cabinRestrictions && filters.cabinRestrictions.length > 0) {
          amadeusRequest.searchCriteria.flightFilters.cabinRestrictions =
            filters.cabinRestrictions;
        }
  ...
  async createFlightOrder(
    orderRequest: AmadeusFlightOrderRequest,
    ...
    try {
      ...
      // Realizar la reserva
      const response =
        await this.makeAuthenticatedRequest<AmadeusFlightOrderResponse>(
          'POST',
   ...
   */
  private validateFlightOrderRequest(
    orderRequest: AmadeusFlightOrderRequest,
  ): void {
    if (!orderRequest.data) {
      throw new HttpException(
        'Datos de reserva requeridos',
        HttpStatus.BAD_REQUEST,
      );
    }

    if (
      !orderRequest.data.flightOffers ||
      orderRequest.data.flightOffers.length === 0
    ) {
      throw new HttpException(
        'Al menos una oferta de vuelo es requerida',
        HttpStatus.BAD_REQUEST,
      );
    }

    if (
      !orderRequest.data.travelers ||
      orderRequest.data.travelers.length === 0
    ) {
      throw new HttpException(
        'Al menos un viajero es requerido',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    orderRequest.data.travelers.forEach((traveler, index) => {
      ...
      if (
        ...
        traveler.contact.phones.length === 0
      ) {
        throw new HttpException(
